...
parse_tileset :: proc(path: string) -> (t: Tileset, err: bool) {
	...
	if ok != nil {
		fmt.printfln("ERROR: Tileset `{}` not found in {}", path, os.get_current_directory());
	...
	if tileset.version != EXPECTED_VERSION {
		fmt.printfln("WARNING: expected version `{}`, GOT `{}`, certain things might not work right.", EXPECTED_VERSION, tileset.version);
	}
